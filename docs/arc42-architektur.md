...
| App-lokale Vokabulare (`src/vokabulare/`) | Inhalte, die laut `docs/entwicklung.md` bewusst nicht in den Kern gewandert sind, z. B. Landesvorlagen (`import.meta.glob`, eine Vite-Eigenschaft) und die Berliner Funkrufnamen-Ausnahme (`thw-funkrufname-ort.ts`). |
...
(die Berliner OV-Kennzahlen des Funkrufnamens; Kandidat für den Kern, sobald
weitere Stadtstaaten belegt sind) sowie die Zeichen-Symboldaten.
...
| Funkrufname | Standardisierte Sprechfunkbezeichnung eines Fahrzeugs/einer Einheit (z. B. „Heros Oldenburg 18/13"). In Berlin sprechen alle zwölf Ortsverbände auf „Berlin", der OV steht als führende Kennzahl davor („Heros Berlin 06/22/51"). |
