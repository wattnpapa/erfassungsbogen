...
**Die Rückfrage-Regel.** Eine Aktion, die erfasste Angaben vernichtet, fragt nach — und nennt in der Frage, was sie trifft („Meyer, Jan entfernen?"). Das gilt für die Einträge genauso wie für den Bogen: „Neuer Bogen" und „Entwurf verwerfen" fragen seit je, Person und Fahrzeug verschwanden dagegen still, samt Name, Funktionen, Qualifikationen und Erreichbarkeiten. Der Löschknopf sitzt in der Kopfzeile zwischen Sortierpfeilen und Rollen-Auswahl, der Fehlgriff ist also eingebaut. Die Gegenprobe gehört zur Regel: Ein **leerer** Eintrag geht ohne Dialog (`personLeer`/`fahrzeugLeer` in `hilfen.ts`) — wer eben „+ Person hinzufügen" danebengetippt hat, soll die Karte ohne Rückfrage loswerden, sonst erzieht der Dialog zum Wegklicken und schützt am Ende nichts mehr.

**Die Eintragsnummer-Regel.** Ein wiederholter Eintrag trägt seine Stelle in der Liste sichtbar über dem Kopf („Person 3 von 12"), klein und zurückgenommen. Solange keine Namen stehen, sind zwölf Karten optisch identisch — und die Stelle ist genau das, was die Sortierknöpfe verschieben und der Löschknopf trifft. Für Hilfstechnik stand die Nummer schon im `aria-label`; sichtbar fehlte sie.

**Die Reihenfolge-Regel.** Wo die Stelle in einer Liste Inhalt ist und nicht Anzeige, gehört an jeden Eintrag ein Griff, der ihn bewegt. In der Personalliste gilt die erste Person als Ansprechpartner/in — sie trägt dafür eine Marke im Eintragskopf, sonst bliebe die Regel stumm und das Sortieren daneben unerklärt. Bewegt wird über zwei Pfeilknöpfe (oben/unten) und, ab dem zweiten Eintrag, einen dritten an die erste Stelle; Ziehen mit der Maus gibt es nicht, weil der Bogen am Einsatzort auf dem Telefon ausgefüllt wird und Drag-and-drop weder für die Tastatur noch für den Screenreader ohne eigenen Ersatzweg zu bedienen ist. Der Fokus wandert nach dem Verschieben mit dem Eintrag mit: bliebe er an der Stelle, hebt der zweite Klick die nachgerückte Nachbarzeile.
...
- **Die Kästchenzeile ist das Tippziel, nicht das Kästchen.** Ein Schalter ist ein `<label class="inline">`, das Kästchen und Beschriftung umschließt; das Label trägt darum `min-height: calc(var(--ziel-basis) + var(--ziel))`. Ohne das war die Trefferfläche so hoch wie die Textzeile — gemessen 24 gegen 44 px —, und getroffen wurde nur, wer genau die Schriftzeile erwischte. Die Plattform-Layer setzten das längst (44 px iOS, 48 px Android); im Browser, wo die App meistens läuft, fehlte es. Der eine `<span class="inline">` (Fahrerlaubnis-Zusatz) gruppiert schon zielgroße Felder und nimmt die Mindesthöhe ausdrücklich zurück.
...
- **Checkbox/Radio:** `accent-color: var(--akzent)`; im Feld-Modus 1.7rem groß. Das Tippziel ist die Zeile, nicht das Kästchen (siehe Zielmaß-Regel).
- **Tastatur nach Inhalt:** Ein Feld sagt dem Telefon, welche Tastatur es braucht — `type`/`inputmode` `tel` für Nummern, `email` für Adressen. Ohne das lag der Ziffernblock für die Rückrufnummer der Einheit hinter einer Umschaltung; `type="number"` ist dafür kein Ersatz, Vorwahl-Trennzeichen und führende Null gehören nicht in ein Rechenfeld.
- **Zählen statt tippen:** Wiederkehrende kleine Zahlen bekommen den `Stepper` (−/Zahl/+), nicht das nackte Zahlenfeld — die Stärke der Meldekopf-Schnellerfassung (Führer/Unterführer/Mannschaft), die Unterbringungsplätze M/W/D und die Verpflegungszahlen. Der Fall ist eine Aufnahme im Stehen, mit Handschuh; direkte Eingabe bleibt möglich, nötig ist sie nicht. Errechnete Werte (Gesamt) bleiben Anzeige.
...
- Im Assistenten teilen sich Rücksprung („‹ Startseite") und Anzeige-Umschalter die erste Zeile (`.kopf-oberzeile`); darunter Zeichen, Titel und — in der Meldekopf-Schnellerfassung — die Modus-Marke, dann der Speicherstand, unten die Schrittleiste als Reiterzeile.
- **Der Platzhaushalt des Kopfes ist Teil seiner Gestaltung.** Gemessen im Telefon-Viewport (360 × 640) belegte der Assistenten-Kopf 330 px im Standard- und 390 px im Feld-Modus: sichtbar blieben die Schrittüberschrift und eine Optionszeile, jede Eingabe begann mit Scrollen — im Modus, der „Feld" heißt. Unter 30rem wird er deshalb schmal gesetzt statt gekürzt (kleineres Zeichen, Titel einzeilig, engere Reiter; 254 bzw. 353 px). Alle sechs Schrittnamen bleiben lesbar: eine waagerecht rollende Reiterzeile hätte mehr gespart und dafür Schritt 5 und 6 versteckt — wer den Assistenten selten benutzt, findet dann die Übersicht nicht mehr.
...
**Die Arbeitszustands-Regel.** „Arbeitet" darf nicht wie „gesperrt" aussehen. Ein Knopf, der während seiner Arbeit auf `disabled` läuft, erbt die 0.4 Deckkraft — und damit genau das Bild, mit dem die App „hier geht gerade nichts" sagt. Wer unter Zeitdruck ein PDF anfordert (Setzen samt QR-Code: auf dem Telefon mehrere Sekunden), tippt dann mehrfach oder bricht ab. Der Zustand behält deshalb volle Schrift und starke Linie auf zweiter Fläche (`button.arbeitet:disabled`), trägt `aria-busy` und bekommt eine Statuszeile, die in Worten sagt, was läuft. Keinen Laufbalken und keinen kreisenden Kringel: die Feldgeräte-Regel lässt nichts in Schleife laufen — die Auskunft „es läuft noch" trägt der Satz, nicht die Bewegung.
